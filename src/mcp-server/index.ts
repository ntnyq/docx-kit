...
export async function createDocxKitServer(): Promise<unknown> {
  ...
  // @ts-expect-error — @modelcontextprotocol/sdk is optional; installed separately
  const { McpServer } = await import('@modelcontextprotocol/sdk/server/mcp.js')
  // @ts-expect-error — zod is optional; installed separately
  ...
  server.tool(
    'list_plugins',
    'List all available docx-kit plugins',
    {
  ...
  server.tool(
    'get_plugin_help',
    'Get usage help for a specific docx-kit plugin',
    {
  ...
  server.tool(
    'apply_template',
    'Generate a document schema from an AI template + data',
    {
      data: z.record(z.unknown()).describe('Template parameter values'),
      template: z
        .string()
